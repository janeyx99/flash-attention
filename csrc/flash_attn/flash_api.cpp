...
// Stable-ABI migration: this translation unit uses only torch::stable /
// torch::headeronly APIs (no <torch/extension.h>, ATen, or c10), so the built
// _vllm_fa2_C extension is ABI-stable against PyTorch >= TORCH_TARGET_VERSION.
...
// Dropout uses ATen's default CUDA generator, which has no stable-ABI equivalent
// yet. These includes (and the philox host path below) are kept verbatim but are
// compiled out whenever FLASHATTENTION_DISABLE_DROPOUT is set (as in this build),
// so the stable build never pulls them in. This keeps the file close to upstream.
#include <ATen/cuda/CUDAGeneratorImpl.h>  // For at::PhiloxCudaState / at::CUDAGeneratorImpl
...
#ifndef FLASHATTENTION_DISABLE_DROPOUT
// Validate the opaque philox buffer against the real at::PhiloxCudaState layout
// here, where the type is visible (compiled out when dropout is disabled).
...
void set_params_fprop(Flash_fwd_params &params,
    ...
    params.is_bf16 = q.scalar_type() == th::ScalarType::BFloat16;
...
std::tuple<Tensor, Tensor> set_params_splitkv(Flash_fwd_params &params, const int batch_size,
    ...
    if (p_dropout == 0.0f) {  // SplitKV is not implemented for dropout
        ...
        if (params.num_splits > 1) {
            softmax_lse_accum = torch::stable::new_empty(ref, {params.num_splits, batch_size, num_heads, max_seqlen_q}, th::ScalarType::Float);
            out_accum = torch::stable::new_empty(ref, {params.num_splits, batch_size, num_heads, max_seqlen_q, head_size_rounded}, th::ScalarType::Float);
...
void set_params_alibi(Flash_fwd_params &params, std::optional<Tensor> &alibi_slopes_, int batch_size, int num_heads){
    ...
    if (alibi_slopes_.has_value()) {
        ...
        STD_TORCH_CHECK(alibi_slopes.scalar_type() == th::ScalarType::Float, "ALiBi slopes must have dtype fp32");
        CHECK_DEVICE(alibi_slopes);
        STD_TORCH_CHECK(alibi_slopes.stride(-1) == 1, "ALiBi slopes tensor must have contiguous last dimension");
        STD_TORCH_CHECK(alibi_slopes.dim() == 1 || alibi_slopes.dim() == 2, "ALiBi slopes must be 1D or 2D");
...
mha_fwd(Tensor q,         // batch_size x seqlen_q x num_heads x round_multiple(head_size, 8)
        Tensor k,         // batch_size x seqlen_k x num_heads_k x round_multiple(head_size, 8)
        Tensor v,         // batch_size x seqlen_k x num_heads_k x round_multiple(head_size, 8)
...
mha_varlen_fwd(Tensor q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    ...
    if (paged_KV) {
        ...
        STD_TORCH_CHECK(block_table.scalar_type() == th::ScalarType::Int, "block_table must have dtype torch.int32");
    ...
    if (leftpad_k_.has_value()) {
        ...
        STD_TORCH_CHECK(leftpad_k.scalar_type() == th::ScalarType::Int, "leftpad_k must have dtype int32");
...
std::vector<Tensor>
mha_bwd(Tensor dout,  // batch_size x seqlen_q x num_heads, x multiple_of(head_size_og, 8)
        ...
        Tensor softmax_lse,     // b x h x seqlen_q
    ...
    auto softmax_d = torch::stable::new_empty(q, {batch_size, num_heads, seqlen_q_rounded}, th::ScalarType::Float);
    Tensor dq_accum;
    Tensor dk_accum, dv_accum;
    if (loop) {
        if (!deterministic) {
            dq_accum = torch::stable::new_empty(q, {batch_size, seqlen_q_rounded, num_heads, head_size_rounded}, th::ScalarType::Float);
        } else {
            const int nsplits = (get_num_sm(get_current_device()) + batch_size * num_heads - 1) / (batch_size * num_heads);
            dq_accum = torch::stable::new_zeros(q, {nsplits, batch_size, seqlen_q_rounded, num_heads, head_size_rounded}, th::ScalarType::Float);
        }
    }
...
std::vector<Tensor>
mha_varlen_bwd(Tensor dout,  // total_q x num_heads, x head_size
               ...
               Tensor out,   // total_q x num_heads x head_size
               Tensor softmax_lse,    // h x total_q, softmax logsumexp
    ...
    auto softmax_d = torch::stable::new_empty(q, {num_heads, total_q + 128 * batch_size}, th::ScalarType::Float);
    Tensor dq_accum;
    if (loop) {
        ...
        if (!deterministic) {
            dq_accum = torch::stable::new_empty(q, {total_q + 128 * batch_size, num_heads, head_size_rounded}, th::ScalarType::Float);
        } else {
            const int nsplits = (get_num_sm(get_current_device()) + batch_size * num_heads - 1) / (batch_size * num_heads);
            dq_accum = torch::stable::new_zeros(q, {nsplits, total_q + 128 * batch_size, num_heads, head_size_rounded}, th::ScalarType::Float);
        }
    }
...
mha_fwd_kvcache(Tensor q,                 // batch_size x seqlen_q x num_heads x head_size
    ...
    if (seqlens_k_.has_value()) {
        ...
        STD_TORCH_CHECK(seqlens_k.scalar_type() == th::ScalarType::Int, "seqlens_k must have dtype int32");
    ...
    if (cache_batch_idx_.has_value()) {
        ...
        STD_TORCH_CHECK(cache_batch_idx.scalar_type() == th::ScalarType::Int, "cache_batch_idx must have dtype int32");
